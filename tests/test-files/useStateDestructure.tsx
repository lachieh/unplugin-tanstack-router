...
import { Await, Link, createFileRoute, getRouteApi } from '@tanstack/react-router'
import { TbHeartHandshake } from 'react-icons/tb'
import { Carbon } from '~/components/Carbon'
import { Footer } from '~/components/Footer'
import SponsorPack from '~/components/SponsorPack'
import { startProject } from '~/projects/start'
import type { Framework } from '~/projects'
...
const menu = [
  {
    label: (
      <div className="flex items-center gap-2">
        <CgCornerUpLeft className="text-lg" />
        {' '}
        TanStack
      </div>
    ),
...
export default function VersionIndex() {
  ...
  const _branch = getBranch(startProject, version)
  const [_framework, _setFramework] = React.useState<Framework>('react')
  const [_isDark, setIsDark] = React.useState(true)
